...
Requires Node `>=24`, `pnpm`, and a logged-in Fluid CLI (`fluid login`).

If you generated this project with `fluid widget create <name>`, it is already
renamed and its dependencies are installed. If you started from this repository
directly (`git clone` or GitHub's *Use this template*), run `pnpm install`
first, then rename the package in `package.json` / `index.html` and the
`scope` + `version` in `manifest.ts`.

```bash
fluid widget link          # link this project to a Fluid droplet
pnpm dev                   # run the local builder/preview
fluid widget push          # commit + sync git, then build & publish
```

`fluid widget push` keeps a hosted git repository in sync (auto-commit + push
to `main`) and then runs the build + publish pipeline. Use
`fluid widget clone <package>` to check out an existing widget package's repo
on another machine. Both fall back gracefully when git sync is unavailable.
